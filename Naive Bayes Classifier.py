...
#%%
#Setting up gnb Classifier with 3 n-neighbours
gnb = GaussianNB()
gnb.fit(train_x,train_y)
#%%
#cross validation for accuracy with 5 K-Folds on train
scores_train = cross_val_score(gnb, train_x, train_y, cv=5)
print('Accuracy : ',scores_train.mean()*100,'\n','Deviation : ',scores_train.std())
#%%
#score for accuracy with 5 K-Folds on test
scores_test = gnb.score(test_x, test_y)
print('Accuracy : ',scores_test)
#%%
#Train Classification Reports
visualizer = ClassificationReport(gnb,classes=cat.categories,support=True)
visualizer.score(test_x, test_y)
visualizer.poof()    
#%%
#Train Confusion Matrix
visualizer = ConfusionMatrix(gnb,classes=cat.categories,label_encoder={i:j for i,j in enumerate(list(cat.categories))})
visualizer.score(test_x, test_y)
visualizer.poof()    
#%%
#Train Class Pred Bar graph
visualizer = ClassPredictionError(gnb,classes=cat.categories)
visualizer.score(test_x, test_y)
visualizer.poof()    
#%%
...
scores = gnb.score(test_data_x, test_data_y)
